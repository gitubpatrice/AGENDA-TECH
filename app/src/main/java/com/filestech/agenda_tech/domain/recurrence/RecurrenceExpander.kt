 ...
 *
 * **Cost.** [MAX_SCAN_ITERATIONS] bounds ONE event. The number of events is what an import controls, so
 * a caller that expands many of them in a single pass must also pass an [ExpansionBudget] — see its
 * KDoc. Audit F8 added that parameter to [expand] only; [nextOccurrenceStart] and
 * [lastOccurrenceStartBefore] were the two remaining entry points where N events still cost N times the
 * per-event cap, and they are the ones the reminder scheduler and the search box go through.
 */
